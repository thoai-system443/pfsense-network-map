...
**Interfaces:**
- Consumes: không có
- Produces: `config.apiUrl: string` — đọc từ `window.__CONFIG__.API_URL`, mặc định `http://localhost:8000` khi không có
...
const configured = window.__CONFIG__?.API_URL ?? "http://localhost:8000";
...
Run: `curl -s localhost:8080/config.js`
Expected: `window.__CONFIG__ = { API_URL: "http://localhost:8000" };`

Run: `curl -s -o /dev/null -w '%{http_code}' localhost:8080/some/deep/route`
...
describe("api client", () => {
  it("posts the file as multipart to the configs endpoint", async () => {
    ...
    expect(url).toBe("http://localhost:8000/api/v1/configs");
  ...
  it("builds the topology url from the config id", async () => {
    ...
    expect(spy.mock.calls[0][0]).toBe("http://localhost:8000/api/v1/configs/abc/topology");
  });
...
| Tên | Bắt buộc | Mô tả |
|---|---|---|
| `WEB_PORT` | không | Port host, mặc định 8080 |
| `API_URL` | không | Địa chỉ backend, mặc định `http://localhost:8000` |
...
Mở `http://localhost:8080`, upload `backend/tests/fixtures/vlan_vpn.xml`, rồi kiểm tra bằng mắt:
...
- [ ] `docker compose up -d --build` phục vụ được SPA ở `http://localhost:8080`
