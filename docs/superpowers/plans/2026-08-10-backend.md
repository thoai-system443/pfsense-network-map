...
> **Đã thực thi 2026-08-10. Năm điểm lệch so với nội dung bên dưới, tất cả đều do môi trường thật:**
> 1. `docker-compose.yml` thêm `name: pfsense-network-map-backend`. Compose lấy tên project từ tên thư mục (`backend`), trùng với một project khác trên máy đang chạy `backend-db-1`; container của project kia bị coi là orphan của project này và có thể bị `--remove-orphans` xoá mất.
> 2. Port mặc định 8000 → **8010**. Port 8000 đã bị `vibecloud-backend-app-1` chiếm. `CORS_ORIGINS` đổi sang `http://localhost:8011` cho khớp frontend.
> 3. nginx build từ `nginx/Dockerfile` riêng thay vì dùng thẳng `nginx:alpine`, vì image đó **không có `openssl`**. Cài lúc build để runtime vẫn air-gapped.
> 4. `nginx/entrypoint.sh` kiểm tra hash tường minh. `set -e` không bắt được command substitution hỏng nằm trong tham số `printf`, nên bản gốc ghi ra `.htpasswd` cụt (`changeme:`) và trả 401 cho mọi request mà không báo lỗi.
> 5. `Dockerfile` cài dependency trên một package `app` rỗng trước khi copy code thật, để layer dependency được cache. `test.sh` mount `app/` và `tests/` để mỗi vòng red/green không phải build lại image.
